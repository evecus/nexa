...
type ConfigSection struct {
	...
	// 候选列表：设置页以 chip 形式展开展示、可删除、可添加；启动配置页下拉选项与之同源。
	// null 表示用户从未修改过（前端展示动态默认值）；[] 表示用户已清空（保持为空）。
	BinaryList []string `json:"binary_list"`
	ArgsList   []string `json:"args_list"`
	RunDir               string `json:"run_dir"` // 运行目录，空 = 默认 <数据目录>/run；非空必须为绝对路径
...
func defaultRouterAccessControls() []RouterAccessControl {
	...
	return []RouterAccessControl{
		{
			ID: "router-default-bypass", Enabled: true,
			User:  []string{"nobody", "systemd-network", "systemd-resolve", "_chrony", "sshd"},
			Group: []string{"nogroup", "systemd-network", "systemd-resolve", "_chrony", "_ssh"},
			Cgroup: []string{

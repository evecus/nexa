...
// Credentials 认证凭据（auth bucket 单行存储，替代旧 cred.json）。
...
func (s *Store) init() error {
	return s.db.Update(func(tx *bolt.Tx) error {
		for _, b := range [][]byte{bucketMeta, bucketConf, bucketAuth} {
...
// LoadAuth 读取凭据；不存在返回 (nil, nil)。
func (s *Store) LoadAuth() (*Credentials, error) {
	var raw []byte
	err := s.db.View(func(tx *bolt.Tx) error {
		if b := tx.Bucket(bucketAuth); b != nil {
			raw = b.Get(authRowID)
		}
		return nil
	})
...
// SaveAuth 保存凭据（整体覆盖）。
func (s *Store) SaveAuth(c *Credentials) error {
	raw, err := json.Marshal(c)
	if err != nil {
		return err
	}
	return s.db.Update(func(tx *bolt.Tx) error {
		b, err := tx.CreateBucketIfNotExists(bucketAuth)
		if err != nil {
			return err
		}
		return b.Put(authRowID, raw)
	})
}

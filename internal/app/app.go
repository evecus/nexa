...
func New() (*App, error) {
	...
	// 启动时先按配置中的 run_dir 应用运行目录（设置里为空则用默认 <数据目录>/run）。
	// 非法值（如相对路径）静默忽略，保持默认值；Apply/Reload 时会再次校验并向用户报错。
	if cfg, err := st.Load(); err == nil {
		_ = paths.SetRunDir(cfg.Config.RunDir)
	}
...
// applyRunDir 根据配置应用运行目录（settings 里的 run_dir，空 = 默认 <数据目录>/run）。
// 指定时必须是绝对路径；目录不存在则创建。
// 必须在 Core.Start/Restart 之前调用（core 会把 profile 复制到 paths.RunDir 下）。
func (a *App) applyRunDir(cfg *config.Config) error {
	if err := paths.SetRunDir(cfg.Config.RunDir); err != nil {
		return fmt.Errorf("运行目录设置无效: %w", err)
	}
	if err := os.MkdirAll(paths.RunDir, 0755); err != nil {
...
func (a *App) Apply(cfg *config.Config) error {
	...
	// 应用运行目录（settings.run_dir），再启动核心
	if err := a.applyRunDir(cfg); err != nil {
		return err
	}
